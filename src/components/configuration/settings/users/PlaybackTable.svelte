  ...
  <Table.Root class="mx-auto h-full w-full flex flex-col max-w-[calc(100vw-8.3rem)] max-h-[calc(100vh-310px)] hide-scrollbar overflow-x-auto">
    ...
    >
      <Table.Row class="bg-transparent flex items-center justify-between p-3">
        <Table.Head class="text-[#727272] h-full w-[50px]"><input type="checkbox" name="" id=""></Table.Head>
    ...
    <Table.Body class=" pb-10">
      {#each allUsers as user}
        <Table.Row class="bg-transparent cursor-pointer flex w-max min-w-full items-center justify-between gap-4 mt-4 px-3 rounded-lg  border-2 border-solid border-[#e4e4e4]">
          ...
          {#each playbackFeatures as feature}
            <Table.Cell class="text-[#727272] h-full w-[180px]">
              <input type="checkbox" 
                     checked={user?.features ? user?.features.includes(feature.id) : false} 
      ...
      {/each}
        ...
        <Button class='mr-auto' on:click={handleFeaturesUpdate}>
      save
    </Button>
  </Table.Root>
<!-- </div>/ -->